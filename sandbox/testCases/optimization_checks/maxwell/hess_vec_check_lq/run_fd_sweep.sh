#!/bin/bash
# FD-fallback HessVec sweep (other_decks_fd has no src_gate).
...
mkdir -p logs
ln -sfn other_decks_fd other_decks

for mode in $MODES; do
  for tag in $TAGS; do
    runtag="${tag}_${mode}"
    logfile="mrhyde_hv_${runtag}_fd.log"
    inputfile="input_fd_${runtag}.yaml"
    ...
    echo "=== FD: tag=${tag}, mode=${mode}, base=${base_src} ==="
    np=$(np_for_tag "$tag")
...
rm -f other_decks
echo ""
echo "=== All FD runs complete ==="
ls -la logs/mrhyde_hv_r*_fd.log 2>/dev/null
