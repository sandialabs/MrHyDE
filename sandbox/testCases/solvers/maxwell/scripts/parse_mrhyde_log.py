...
def parse_timing(text):
    ...
    # Find the last TimeMonitor section (full summary at end of run)
    section_pat = r"={50,}\n(.*?TimeMonitor.*?)\n={50,}"
    matches = list(re.finditer(section_pat, text, re.DOTALL))
    if not matches:
        return pd.DataFrame()
    section = matches[-1].group(0)
    ...
    for line in lines:
        if line.startswith("MrHyDE::") or line.startswith("STK_") or \
           ...
           line.startswith("Time ") or line.startswith("Total Time") or \
           line.startswith("MueLu") or line.startswith("RefMaxwell") or \
           line.startswith("Belos"):
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    if not timing.empty:
        sort_col = "MeanOverProcs" if "MeanOverProcs" in timing.columns else "GlobalTime"
        timing = timing.sort_values(sort_col, ascending=False)
