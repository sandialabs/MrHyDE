...
desc = '''hartmann objective record verification
       '''
...
status += its.call('mpiexec -n 1 ../../mrhyde input.yaml >> %s.log' % (root))
status += its.clean_log()

status += its.call('diff -y %s.log %s.gold' % (root, root))
status += its.call('diff -y obj.out obj.out.gold')
status += its.call('diff -y obj_grad.out obj_grad.out.gold')
