...
SubGridExpFEM::SubGridExpFEM(const Teuchos::RCP<MpiComm> & LocalComm_,
...
int SubGridExpFEM::addMacro(DRV & macronodes_, Kokkos::View<int****,HostDevice> & macrosideinfo_,
  ...
  
  {
    ...
    for (size_t e=0; e<currcells[0].size(); e++) {
      ...
      Kokkos::View<int****,HostDevice> subsideinfo("subcell side info", currcells[0][e]->numElem, sideinfo.extent(1),
                                                   sideinfo.extent(2), sideinfo.extent(3));
      ...
      for (size_t c=0; c<currcells[0][e]->numElem; c++) { // number of elem in cell
        for (size_t i=0; i<sideinfo.extent(1); i++) { // number of variables
          for (size_t j=0; j<sideinfo.extent(2); j++) { // number of sides per element
            for (size_t k=0; k<sideinfo.extent(3); k++) { // boundary information
  ...
  
  {
    ...
    int numLocalBoundaries = macrosideinfo_.extent(2);
    ...
    Kokkos::View<int**,AssemblyDevice> currbcs("boundary conditions",sideinfo.extent(1),
                                               macrosideinfo_.extent(2));
    for (size_t i=0; i<sideinfo.extent(1); i++) { // number of variables
      for (size_t j=0; j<macrosideinfo_.extent(2); j++) { // number of sides per element
        currbcs(i,j) = 5;
      }
    }
  ...
  
  {
    ...
    if (first_time) {
      ...
      disc = Teuchos::rcp( new discretization(settings, LocalComm, mesh, orders, types) );
      ...
      for (size_t e=0; e<bCells[0].size(); e++) {
        ...
        bCells[0][e]->orientation = boundaryCells[0][e]->orientation;
      }
    }
  }
  ...
  // Set up the linear algebra objects
  {
    ...
    if (first_time) {
      ...
      for (size_t e=0; e<cells[0].size(); e++) {
        ...
        int numDOF = currcells[0][0]->GIDs.extent(1);
  ...
  if (first_time) { // first time through
    ...
    int nmacroDOF = macroGIDs.extent(1);
  ...
  
  {
    ...
    if (multiscale_method != "mortar" ) {
      ...
      for (size_t e=0; e<cells[block].size(); e++) {
        ...
        DRV sref_ip_tmp("sref_ip_tmp",numElem, ip.extent(1), ip.extent(2));
        DRV sref_ip("sref_ip",ip.extent(1), ip.extent(2));
        CellTools::mapToReferenceFrame(sref_ip_tmp, ip, macronodes[block], *macro_cellTopo);
      ...
      for (size_t e=0; e<boundaryCells[block].size(); e++) {
        ...
        for (size_t c=0; c<numElem; c++) {
          ...
          CellTools::mapToReferenceFrame(sref_side_ip_tmp, side_ip_e, macronodes[block], *macro_cellTopo);
...
void SubGridExpFEM::addMeshData() {
...
void SubGridExpFEM::subgridSolver(Kokkos::View<ScalarT***,AssemblyDevice> gl_u,
...
void SubGridExpFEM::storeFluxData(Kokkos::View<ScalarT***,AssemblyDevice> lambda, Kokkos::View<AD**,AssemblyDevice> flux) {
...
void SubGridExpFEM::sacadoizeParams(const bool & seed_active, const int & num_active_params) {
...
void SubGridExpFEM::subGridNonlinearSolver(Teuchos::RCP<LA_MultiVector> & sub_u,
  ...
  while (iter < sub_maxNLiter && resnorm_scaled[0] > sub_NLtol) {
    ...
    int numDOF = cells[usernum][0]->GIDs.extent(1);
...
void SubGridExpFEM::computeSubGridSolnSens(Teuchos::RCP<LA_MultiVector> & d_sub_u,
  ...
  if (compute_sens) {
    ...
    if (multiscale_method != "mortar") {
      ...
      int snumDOF = cells[usernum][0]->GIDs.extent(1);
      ...
      for (size_t e=0; e<boundaryCells[usernum].size(); e++) {
        ...
        int snumDOF = boundaryCells[usernum][e]->GIDs.extent(1);
    ...
    if (multiscale_method != "mortar") {
      ...
      for (size_t e=0; e<cells[usernum].size(); e++) {
        ...
        int snumDOF = cells[usernum][e]->GIDs.extent(1);
        int anumDOF = cells[usernum][e]->auxGIDs.extent(1);
      ...
      for (size_t e=0; e<boundaryCells[usernum].size(); e++) {
        ...
        int numElem = boundaryCells[usernum][e]->numElem;
        int snumDOF = boundaryCells[usernum][e]->GIDs.extent(1);
        int anumDOF = boundaryCells[usernum][e]->auxGIDs.extent(1);
...
void SubGridExpFEM::updateFlux(const Teuchos::RCP<LA_MultiVector> & u,
...
void SubGridExpFEM::setInitial(Teuchos::RCP<LA_MultiVector> & initial,
...
//Kokkos::View<ScalarT**,AssemblyDevice> SubGridExpFEM::computeError(const ScalarT & time, const int & usernum) {
Kokkos::View<ScalarT***,AssemblyDevice> SubGridExpFEM::computeError(const vector<string> & error_types,
...
Kokkos::View<AD*,AssemblyDevice> SubGridExpFEM::computeObjective(const string & response_type, const int & seedwhat,
...
void SubGridExpFEM::writeSolution(const string & filename, const int & usernum) {
...
void SubGridExpFEM::addSensors(const Kokkos::View<ScalarT**,HostDevice> sensor_points, const ScalarT & sensor_loc_tol,
...
Teuchos::RCP<LA_CrsMatrix>  SubGridExpFEM::getProjectionMatrix() {
...
pair<Kokkos::View<int**,AssemblyDevice>, vector<DRV> > SubGridExpFEM::evaluateBasis2(const DRV & pts) {
  ...
  for (size_t e=0; e<cells[0].size(); e++) {
    ...
    for (int c=0; c<numElem;c++) {
      ...
      CellTools::mapToReferenceFrame(refpts, pts, cnodes, *cellTopo);
      CellTools::checkPointwiseInclusion(inRefCell, refpts, *cellTopo, 0.0);
  ...
  for (size_t i=0; i<numpts; i++) {
    ...
    CellTools::mapToReferenceFrame(refpt_buffer, cpt, cnodes, *cellTopo);
...
pair<Kokkos::View<int**,AssemblyDevice>, vector<DRV> > SubGridExpFEM::evaluateBasis(const DRV & pts) {
...
Teuchos::RCP<LA_CrsMatrix>  SubGridExpFEM::getEvaluationMatrix(const DRV & newip, Teuchos::RCP<LA_Map> & ip_map) {
...
Kokkos::View<GO**,HostDevice> SubGridExpFEM::getCellGIDs(const int & cellnum) {
...
void SubGridExpFEM::updateParameters(vector<Teuchos::RCP<vector<AD> > > & params, const vector<string> & paramnames) {
...
Kokkos::View<ScalarT**,AssemblyDevice> SubGridExpFEM::getCellFields(const int & usernum, const ScalarT & time) {
...
void SubGridExpFEM::performGather(const size_t & b, const vector_RCP & vec,
...
void SubGridExpFEM::performBoundaryGather(const size_t & b, const vector_RCP & vec,
                               const size_t & type, const size_t & entry) const {
...
void SubGridExpFEM::updateMeshData(Kokkos::View<ScalarT**,HostDevice> & rotation_data) {
  for (size_t b=0; b<cells.size(); b++) {
    for (size_t e=0; e<cells[b].size(); e++) {
      int numElem = cells[b][e]->numElem;
      for (int c=0; c<numElem; c++) {
