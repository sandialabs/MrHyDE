...
hartmann::hartmann(Teuchos::ParameterList &settings, const int &dimension_)
...
void hartmann::defineFunctions(Teuchos::ParameterList &fs,
...
void hartmann::volumeResidual()
{

    Vista hartmannNum, source_u;
    {
    ...
    { // U equation
        ...
        parallel_for(
            ...
            KOKKOS_LAMBDA(const int elem) {
                for (size_type pt = 0; pt < basis.extent(2); pt++)
                {
                    ...
                    AD F = hartmannNum(elem, pt)*db_dx(elem, pt) - source_u(elem, pt);
                    Fx *= wts(elem, pt);
                    F  *= wts(elem, pt);
        ...
        parallel_for(
            ...
            KOKKOS_LAMBDA(const int elem) {
                ...
                {
                    AD Fx = -db_dx(elem, pt);
                    AD F = hartmannNum(elem, pt)*du_dx(elem, pt);
                    Fx *= wts(elem, pt);
                    F  *= wts(elem, pt);
                    for (size_type dof = 0; dof < basis.extent(1); dof++)
                    {
...
void hartmann::boundaryResidual()
{
...
void hartmann::setWorkset(Teuchos::RCP<workset> &wkset_)
{
