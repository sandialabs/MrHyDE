...
void VFP1d2v<EvalT>::defineFunctions(Teuchos::ParameterList & fs,
    ...
    functionManager->addFunction("source_H",fs.get<string>("source_H","0.0"),"ip");
        ...
        parallel_for("VFP 0d2v H residual",
            ...
            for (size_type dof=team.team_rank(); dof<basis.extent(1); dof+=team.team_size() ) {
                for (size_type pt=0; pt<basis.extent(2); ++pt ) {
                    ...
                    //res(elem,off(dof)) += q_H(elem,pt)/m_H*Ex*dHdv1(elem,pt)*wts(elem,pt)*basis_grad(elem,dof,pt,0);
                    //res(elem,off(dof)) += q_H(elem,pt)/m_H*Ey*dHdv2(elem,pt)*wts(elem,pt)*basis_grad(elem,dof,pt,1);
                    // Collisonal terms
                    //AD col_HC = 0.0;
        ...
        parallel_for("Thermal volume resid 3D part 1",
            ...
            for (size_type dof=team.team_rank(); dof<basis.extent(1); dof+=team.team_size() ) {
                for (size_type pt=0; pt<basis.extent(2); ++pt ) {
                    ...
                    //res(elem,off(dof)) += q_C(elem,pt)/m_C*Ex*dCdv1(elem,pt)*wts(elem,pt)*basis_grad(elem,dof,pt,0);
                    //res(elem,off(dof)) += q_C(elem,pt)/m_C*Ey*dCdv2(elem,pt)*wts(elem,pt)*basis_grad(elem,dof,pt,1);
                }
            }
        });
    }
        ...
        parallel_for("Thermal volume resid 3D part 1",
            ...
            for (size_type dof=team.team_rank(); dof<basis.extent(1); dof+=team.team_size() ) {
                for (size_type pt=0; pt<basis.extent(2); ++pt ) {
                    ...
                    //res(elem,off(dof)) += q_G(elem,pt)/m_G*Ex*dGdv1(elem,pt)*wts(elem,pt)*basis_grad(elem,dof,pt,0);
                    //res(elem,off(dof)) += q_G(elem,pt)/m_G*Ey*dGdv2(elem,pt)*wts(elem,pt)*basis_grad(elem,dof,pt,1);
                }
            }
        });
    }
...
template<class EvalT>
std::vector< std::vector<string> > VFP1d2v<EvalT>::setupIntegratedQuantities(const int & spaceDim) {
