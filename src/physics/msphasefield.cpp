...
AD msphasefield::getDirichletValue(const string & var, const ScalarT & x, const ScalarT & y, const ScalarT & z,
...
ScalarT msphasefield::getInitialValue(const string & var, const ScalarT & x, const ScalarT & y, const ScalarT & z,
...
Kokkos::View<ScalarT**,AssemblyDevice> msphasefield::getInitial(const DRV & ip, const string & var,
...
ScalarT msphasefield::trueSolution(const string & var, const ScalarT & x, const ScalarT & y, const ScalarT & z,
...
Kokkos::View<AD***,AssemblyDevice> msphasefield::response(Kokkos::View<AD****,AssemblyDevice> local_soln,
...
Kokkos::View<AD***,AssemblyDevice> msphasefield::target(const DRV & ip, const ScalarT & time) {
  int numip = ip.dimension(1);
...
std::vector<string> msphasefield::extraFieldNames() const {
  std::vector<string> ef;
  ef.push_back("interface");
  return ef;
}

vector<Kokkos::View<ScalarT***,AssemblyDevice> > msphasefield::extraFields(const DRV & ip, const ScalarT & time) const {
...
vector<Kokkos::View<ScalarT***,AssemblyDevice> > msphasefield::extraCellFields(const DRV ip, const ScalarT & time) const {
  std::vector<Kokkos::View<ScalarT***,AssemblyDevice> > ef;
  std::vector<AD>  phi;
  AD dval;
  AD phiInterface;
