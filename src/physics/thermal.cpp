  ...
  parallel_for("Thermal volume resid 3D part 1",
    ...
    for (size_type dof=team.team_rank(); dof<basis.extent(1); dof+=team.team_size() ) {
      for (size_type pt=0; pt<basis.extent(2); ++pt ) {
        ...
        if (have_advection) {
          if (spaceDim == 1) {
            res(elem,off(dof)) += bx(elem,pt)*dTdx(elem,pt)*wts(elem,pt)*basis(elem,dof,pt,0);
          }
          else if (spaceDim == 2) {
            res(elem,off(dof)) += (bx(elem,pt)*dTdx(elem,pt) + by(elem,pt)*dTdy(elem,pt))*wts(elem,pt)*basis(elem,dof,pt,0);
          }
          else {
            res(elem,off(dof)) += (bx(elem,pt)*dTdx(elem,pt) + by(elem,pt)*dTdy(elem,pt) + bz(elem,pt)*dTdz(elem,pt))*wts(elem,pt)*basis(elem,dof,pt,0);
          }
        }
      }
    }
...
void thermal<EvalT>::boundaryResidual() {
  ...
  if (bcs(T_num,cside) == "Neumann") { // Neumann BCs
    ...
    if (bcs(T_num,cside) == "weak Dirichlet") {
      ...
      auto vbdata = wkset->getSolutionField("aux T");
    ...
    parallel_for("Thermal bndry resid wD",
      ...
      if (dim == 1) {
        for (size_type dof=team.team_rank(); dof<basis.extent(1); dof+=team.team_size() ) {
          for (size_type pt=0; pt<basis.extent(2); ++pt ) {
            res(elem,off(dof)) += epen/h(elem)*diff_side(elem,pt)*(T(elem,pt)-bdata(elem,pt))*wts(elem,pt)*basis(elem,dof,pt,0);
...
void thermal<EvalT>::computeFlux() {
  ...
  ScalarT epen = 10.0;
  {
    ...
    auto lambda = wkset->getSolutionField("aux T");
...
std::vector< std::vector<string> > thermal<EvalT>::setupIntegratedQuantities(const int & spaceDim) {
  ...
  std::vector<string> IQ = {"T","thermal vol total T","volume"};
  integrandsNamesAndTypes.push_back(IQ);

  IQ = {"T","thermal bnd total T","boundary"};
  integrandsNamesAndTypes.push_back(IQ);
