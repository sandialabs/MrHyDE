...
AnalysisManager::AnalysisManager(const Teuchos::RCP<MpiComm> & comm_,
                                 ...
                                 Teuchos::RCP<SolverManager<SolverNode> > & solver, // AquiROL2
                                 Teuchos::RCP<PostprocessManager<SolverNode> > & postproc,
                                 Teuchos::RCP<ParameterManager<SolverNode> > & params) : // AquiROL2
...
void AnalysisManager::run() {
  ...
  if (analysis_type == "forward") {
    ...
    if (bound_vars) {
      ...
      output = algo.run(*x, *obj, (comm_->getRank() == 0)); //only processor of rank 0 prints out // AquiROL
    }
    ...
    */
    ...
    // Generate data and get objective
    obj = Teuchos::rcp( new ROL::Objective_MILO<RealT> (solver_, postproc_, params_)); // AquiROL2

    MrHyDE_OptVector xtmp = params_->getCurrentVector(); // AquiROL2
