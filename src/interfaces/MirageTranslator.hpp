  ...
  void MirageTranslator(Teuchos::RCP<Teuchos::ParameterList> & settings, const std::string & filename) {
    ...
    */
    ...
    if (use_explicit) {
      ...
      settings->sublist("Solver").set<bool>("minimize memory",mirage_settings->sublist("MrHyDE Options").get<bool>("minimize memory",true));
      ...
      settings->sublist("Solver").set<double>("storage proportion",mirage_settings->sublist("MrHyDE Options").get<double>("storage proportion",1.0));
    }
    else {
